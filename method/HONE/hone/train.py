"""Training and dev-evaluation driver for CARVE."""
...
from hone.evaluate import score, to_oneie_record, write_jsonl
from hone.model import CarveModel
...
def predict(model, loader, windows, device, amp_dtype, tau=0.0, min_len=1, merge_gap=0):
    ...
    not ship.
    """
    from hone.decode import apply_decoding_rules
...
def records_at(cands, tau, min_len, merge_gap=0):
    ...
    return [to_oneie_record(c["sent_id"], c["tokens"], c["etype"],
                            apply_decoding_rules(c["conf"], {"_": tau}, merge_gap, min_len, c["n"]),
                            c["aao"], c["trig"])
            for c in cands]
...
def run(cfg):
    ...
    for ep in range(1, cfg["epochs"] + 1):
        ...
        # The sweep only needs Arg-C, which does not depend on ROUGE-L (the AAO
        # head is never thresholded), so score the grid on the fast path and run
        # the full metric once at the selected threshold. Selection is unchanged.
        best_f, recs, sel_tau = -1.0, None, None
        for t in cfg.get("dev_tau_grid", [0.3, 0.4, 0.5, 0.6, 0.7, 0.8]):
            r = records_at(cands, t, cfg.get("dev_min_len", 3))
            f = score(r, gold_path, with_rouge=False)["arg_c_iou"]["f1"]
            if f > best_f:
                best_f, recs, sel_tau = f, r, t
