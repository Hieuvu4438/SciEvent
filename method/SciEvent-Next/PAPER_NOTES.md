# PAPER_NOTES.md — SciEvent-Next: exact method and experiment record
...
Name used internally: **SciEvent-Next**. Architecture tag: multi-task word-level
...
| **SciEvent-Next (ours)** | **63.46** | 53.35 | **57.95** | **55.30** | **46.47** | **50.48** |
...
python3 scripts/full_tables.py # regenerates §6.2–6.4 and §6.8–6.10
```

Artefacts: `artifacts/decoding_rules.json` (frozen rule),
`artifacts/decoding_rules.docsplit.json`, `artifacts/preds/test_preds_s{42,13,101}.jsonl`,
`artifacts/preds/docsplit_test_s{42,13,101}.jsonl`,
`artifacts/runs/<run>/log.json` (per-epoch history), `artifacts/EXPERIMENTS.md`.
