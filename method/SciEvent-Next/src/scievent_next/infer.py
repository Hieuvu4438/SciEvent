...
from scievent_next.evaluate import prf, score, to_oneie_record, write_jsonl  # noqa: E402
from scievent_next.model import SciEventNext  # noqa: E402
from scievent_next.train import WindowDataset, collate  # noqa: E402
...
def posteriors(ckpts, model_name, windows, tok, max_len=640, batch_size=16, device="cuda",
    ...
    heads = _inspect(ckpts[0])
    use_crf = use_crf or heads["use_crf"]
    model = SciEventNext(model_name, len(ROLE_LABELS), len(AAO_LABELS), len(EVENT_TYPES),
...
def apply_rules(spans, tau, merge_gap, min_len, n_tokens):
...
def span_rescore(ckpts, model_name, windows, tok, spans_per_window, max_len=640,
    ...
    model = SciEventNext(model_name, len(ROLE_LABELS), len(AAO_LABELS), len(EVENT_TYPES),
                         use_crf=use_crf or _inspect(ckpts[0])["use_crf"], use_span_role=True).to(device)
    for ck in ckpts:
        model.load_state_dict(torch.load(ck, map_location=device))
        model.eval()
        for batch in dl:
...
def decode_spans(windows, role_p, tau, merge_gap, min_len):
    return [apply_rules(spans_with_conf(role_p[i], ID2ROLE), tau, merge_gap, min_len, len(w.tokens))
...
def build_records(windows, role_p, aao_p, type_p, tau, merge_gap, min_len, override=None):
    ...
    for i, w in enumerate(windows):
        n = len(w.tokens)
        rs = apply_rules(spans_with_conf(role_p[i], ID2ROLE), tau, merge_gap, min_len, n)
...
def tune(windows, role_p, aao_p, type_p, gold_path, verbose=True, override=None, per_role=False):
...
if __name__ == "__main__":
    ...
    if a.tune:
        best = tune(windows, rp, ap_, tp, gold, override=override, per_role=a.per_role)
