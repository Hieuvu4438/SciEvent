"""SciEvent-Next model: a multi-task word-level span tagger.
...
from scievent_next.crf import CRF
...
class SciEventNext(nn.Module):
    def __init__(
        ...
        if use_lstm:
            self.lstm = nn.LSTM(h, h // 2, num_layers=1, batch_first=True, bidirectional=True)
    ...
    def word_states(self, input_ids, attention_mask, word_index, word_mask):
        ...
        if self.use_lstm:
            w = self.lstm(w)[0] * word_mask.unsqueeze(-1)
