...
from carve.evaluate import prf_from_counts, score

HERE = repo_root()
SPLIT = sys.argv[1] if len(sys.argv) > 1 else "test"
GLOB = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "preds", "test_s*.jsonl")
...
print(f"{'CARVE (ours)':24s} | {ip[0]:7.2f} {ir[0]:7.2f} {i1[0]:8.2f} | {cp[0]:7.2f} {cr[0]:7.2f} {c1[0]:8.2f}")
...
print(f"{'CARVE (ours)':24s} | {rp[0]:7.2f} {rr[0]:7.2f} {r1[0]:8.2f}")
...
for name, key in [("PER-DOMAIN", "domain_iou"), ("PER-EVENT-TYPE", "eventtype_iou")]:
    ...
    for k in keys:
        fi = [prf_from_counts(m[key]["Arg_I"][k])[2] for m in mets]
        fc = [prf_from_counts(m[key]["Arg_C"][k])[2] for m in mets]
...
for r in sorted(mets[0]["rolewise_iou"]):
    vals = [prf_from_counts(m["rolewise_iou"][r]) for m in mets]
